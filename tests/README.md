...
| `Real out-of-proc net10 hosts keep state isolated` | 啟動真 `Akka.Proc.Supervisor`、建立 `ActorSystem` client、組出 `FsiMcpService` with real proc/fsi clients | 驗證 `create_fsi_host(net10)` 建出的不同 host 真的隔離，非 fake supervisor | 建兩個 host、各自建同名 session、各自 execute/evaluate | 建 host 前先等 supervisor health；建 session/execute/evaluate 都帶 retry | 每個 host 建完都輪詢 `ProcSupervisor` snapshot 直到有 `fsiSupervisorPath` | 列 host/session 狀態並驗證值 101/202 | stop child procs、terminate actor system、kill supervisor process |
